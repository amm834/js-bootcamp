...
router.post('/admin/products/new', requireAuth, upload.single('image'), [requireTitle, requirePrice], handleErrors(productNewTempalte), async (req, res) => {
	let imageUrl;
...
router.post('/admin/products/:id/edit', upload.single('image'), [requireTitle, requirePrice],
	...
	handleErrors(productEditTempalte, async req => {
		...
		const {id} = req.params;
		const {title, price} = req.body;
		await productRepo.update(id, {title, price})
		res.redirect('/admin/products')
	})
